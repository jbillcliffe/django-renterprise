...
class OrderCustomerList(ListView):
    paginate_by = 5
    #context_object_name = "order_customer_list"
    #customer = get_object_or_404(Customer, customer_token=customer_token)
    #queryset = Order.objects.filter(customer=customer)
    #template_name = "orders/order_customer_list.html"
    ...
    #    return context
    #pk_url_kwarg = 'customer_token'
    #
    #def get_queryset(self, **kwargs):
    #    customer = get_object_or_404(Customer, customer_token=kwargs)
    #   qs = Utilizationtable.objects.filter(pathname=kwargs)
     #   return self.model.objects.filter(customer_token=self.kwargs['customer_token'])

def order_customer_view(request, customer_token, id):
    """
    Function to view a singular order after being selected.
    """
    template_name = "orders/order_customer_view.html"
    ...
    return render(
        request,
        "orders/order_customer_view.html",
        {
            "item":obj,
            "class_var":"Customers",
        },
