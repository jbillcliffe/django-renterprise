...
class CustomerNotesList(ListView):
    ...
    #This pagination method will automatically send my list data as "customernote_list"
    
    def get_queryset(self):
        self.customer = get_object_or_404(Customer, customer_token=self.kwargs["customer_token"])
        return CustomerNote.objects.filter(customer=self.customer).order_by("created_on")
...
class CustomerOrderList(ListView):
    ...
    def get_queryset(self):
        self.customer = get_object_or_404(Customer, customer_token=self.kwargs["customer_token"])
        return Order.objects.filter(customer=self.customer).order_by("start_date")
...
class OrderNotesList(ListView):
    ...
    def get_queryset(self):
        ...
        return OrderNote.objects.filter(order=self.order).order_by("created_on")
    ...
    def get_context_data(self, **kwargs):
        queryset = self.get_queryset()
        get_customer = get_object_or_404(Customer, customer_token=self.kwargs["customer_token"])
